...
func (k Keeper) SetId(ctx context.Context, id types.Id) {
	...
	store.Set(types.IdKey(id.Did), uniquekey)

	store.Set(types.IdKey(id.Creator), uniquekey)

	store.Set(types.IdKey(id.Username), uniquekey)
}
...
func (k Keeper) UpdateUserId(ctx context.Context, id types.Id, oldCreator string) {
	...
	store.Delete(types.IdKey(oldCreator))
...
func (k Keeper) GetIdByDidorUsernameorCreator(
	...
	uniquekey := store.Get(types.IdKey(key))
	if uniquekey == nil {
		return val, false
	}
